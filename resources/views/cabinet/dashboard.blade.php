...
<div class="card mb-8 {{ $plan->code === 'pro' ? 'border-2 border-brand' : '' }}">
    ...
    <!-- Active Discounts -->
    @php $activeDiscounts = $currentCompany->discounts()->active()->get(); @endphp
    @if($activeDiscounts->isNotEmpty())
    <div class="mt-4 pt-4 border-t border-brand-border">
        <div class="text-body-s font-semibold text-text-muted mb-2">{{ __('Your discounts') }}:</div>
        <div class="flex flex-wrap gap-2">
            @foreach($activeDiscounts as $discount)
            <span class="inline-flex items-center gap-1 px-3 py-1 bg-success/10 text-success rounded-full text-body-s font-semibold">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M5 2a2 2 0 00-2 2v14l3.5-2 3.5 2 3.5-2 3.5 2V4a2 2 0 00-2-2H5zm2.5 3a1.5 1.5 0 100 3 1.5 1.5 0 000-3zm6.207.293a1 1 0 00-1.414 0l-6 6a1 1 0 101.414 1.414l6-6a1 1 0 000-1.414zM12.5 10a1.5 1.5 0 100 3 1.5 1.5 0 000-3z" clip-rule="evenodd"/>
                </svg>
                {{ $discount->type === 'percent' ? "-{$discount->value}%" : '-'.number_format($discount->value, 0, '', ' ').' UZS' }}
                &nbsp;·&nbsp;
                {{ $discount->target_label }}
                @if($discount->ends_at)
                &nbsp;· {{ __('until') }} {{ $discount->ends_at->format('d.m.Y') }}
                @endif
            </span>
            @endforeach
        </div>

...
<div class="mb-8">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-h1 font-heading">{{ __('Edit Inbound') }} #{{ $inbound->reference }}</h1>
            <p class="text-body-m text-text-muted mt-2">{{ __('Modify inbound details and items') }}</p>
        </div>
        <a href="{{ route('cabinet.inbounds.show', ['inbound' => $inbound]) }}" class="btn btn-secondary min-h-[44px]">
            ← {{ __('Cancel') }}
        </a>
    </div>
</div>

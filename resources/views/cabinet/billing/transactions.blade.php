            ...
            <table class="w-full responsive-table">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-brand-border/50">
                        <td class="py-3 text-body-s" data-label="{{ __('Date') }}">{{ $tx->created_at->format('d.m.Y H:i') }}</td>
                        <td class="py-3" data-label="{{ __('Type') }}"><span class="badge {{ $tx->getTypeBadgeClass() }}">{{ $tx->getTypeLabel() }}</span></td>
                        <td class="py-3" data-label="{{ __('Description') }}">{{ $tx->description }}</td>
                        <td class="py-3 text-right font-semibold {{ $tx->amount >= 0 ? 'text-success' : 'text-error' }}" data-label="{{ __('Amount') }}">
                            {{ $tx->amount >= 0 ? '+' : '' }}{{ number_format($tx->amount, 0, '', ' ') }}
                        </td>
                        <td class="py-3 text-right text-body-s text-text-muted" data-label="{{ __('Balance After') }}">

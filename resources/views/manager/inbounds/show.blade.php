...
@section('title', 'Приёмка #' . $inbound->id)

@section('content')
<div class="mb-8">
    <a href="{{ route('manager.inbounds.index') }}" class="text-brand hover:underline text-body-s">&larr; Назад к списку</a>
    <h2 class="text-h2 font-heading mt-4">Приёмка #{{ $inbound->id }}</h2>
</div>
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-card border border-brand-border p-6 mb-6">
            ...
            <dl class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <dt class="text-body-s text-text-muted">Ожидаемая дата</dt>
                    <dd>{{ $inbound->expected_at ? $inbound->expected_at->format('d.m.Y') : '-' }}</dd>
                </div>
                @if($inbound->received_at)
                <div>
                    <dt class="text-body-s text-text-muted">Дата приёмки</dt>
                ...
                <table class="w-full">
                    <thead class="bg-bg-soft">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right text-body-s font-semibold text-text-muted">Ожидается</th>
                            <th class="px-4 py-2 text-right text-body-s font-semibold text-text-muted">Принято</th>
                        </tr>
                    </thead>
            ...
            <table class="w-full">
                <thead class="bg-bg-soft">
                    <tr>
                        <th class="px-4 py-2 text-left text-body-s font-semibold text-text-muted">SKU</th>
                        <th class="px-4 py-2 text-left text-body-s font-semibold text-text-muted">Название</th>
                        <th class="px-4 py-2 text-right text-body-s font-semibold text-text-muted">Ожидалось</th>
                        <th class="px-4 py-2 text-right text-body-s font-semibold text-text-muted">Принято</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-brand-border">
    ...
    <div>
        <div class="bg-white rounded-card border border-brand-border p-6">
            ...
            <p class="text-body-s text-text-muted">
                Приёмка завершена {{ $inbound->received_at->format('d.m.Y H:i') }}
            </p>
            @elseif(in_array($inbound->status, ['draft', 'submitted']))
            <p class="text-body-s text-text-muted">
                Заполните количество принятых товаров и нажмите "Подтвердить приёмку"
            </p>
            @endif
        </div>
    </div>
